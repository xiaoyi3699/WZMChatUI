...
- (UIImage *)senderBubbleImage {
    if (_senderBubbleImage == nil) {
        UIImage *image = [LLChatHelper otherImageNamed:@"ll_chat_bj2"];
...
- (UIImage *)receiverBubbleImage {
    if (_receiverBubbleImage == nil) {
        UIImage *image = [LLChatHelper otherImageNamed:@"ll_chat_bj1"];
        CGSize size = image.size;
...
+ (UIImage *)otherImageNamed:(NSString *)name {
    ...
    LLChatHelper *helper = [LLChatHelper shareInstance];
...
- (NSString *)lxhPath {
    if (_lxhPath == nil) {
        NSString *emoticon = [[NSBundle mainBundle] pathForResource:@"emoticon" ofType:@"bundle"];
